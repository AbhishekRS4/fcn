...
class FCN:
    ...
    def _get_conv2d_layer(self, input_tensor, num_filters, kernel_size, strides, name="conv"):
        conv_2d_layer = tf.layers.conv2d(inputs=input_tensor, filters=num_filters, kernel_size=kernel_size,
            strides=strides, padding=self._padding, data_format=self._data_format, kernel_initializer=self._initializer, name=name)
    ...
    def _get_conv2d_transpose_layer(self, input_tensor, num_filters, kernel_size, strides, name="conv_tr"):
        conv_2d_tr_layer = tf.layers.conv2d_transpose(inputs=input_tensor, filters=num_filters, kernel_size=kernel_size,
            strides=strides, padding=self._padding, data_format=self._data_format, kernel_initializer=self._initializer, name=name)
        return
